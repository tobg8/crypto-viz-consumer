...
func (lr *listingRepository) PostListing(c common.ListingDB, cID string) error {
	args := lr.Called(c, cID)
	return args.Error(0)
}

func (lr *listingRepository) QueryListingByKafkaID(kafkaID string) (string, error) {
	args := lr.Called(kafkaID)
...
func TestNewListingUsecase(t *testing.T) {
	t.Run("nominal", func(t *testing.T) {
		...
		in := repository.NewListingRepository(pc)
...
func TestTransformListingEventToListingDB(t *testing.T) {
	...
	t.Run("nominal", func(t *testing.T) {
		...
		priceString := strconv.FormatFloat(in.CurrentPrice, 'g', 5, 64)
		expect := common.ListingDB{
			...
			AtlChangePercentage:          01,
			AtlDate:                      time.Time{},
			KafkaID:                      in.ID + in.Name + priceString,
		}
...
func TestPostListing(t *testing.T) {
	t.Run("listing already exist, return error and empty id", func(t *testing.T) {
		lr := new(listingRepository)
		lu := &listingUsecase{listingRepo: lr}
		...
		post := common.ListingDB{
			...
			FullyDilutedValuation: 12828.77,
			TotalVolume:           122,
			KafkaID:               "superkafkaID",
		}

		cID := "xazheauzuheu"
		lr.On("QueryListingByKafkaID", post.KafkaID).Return(cID, nil)

		id, err := lu.postListing(post, cID)
		assert.Error(t, err)
		assert.Equal(t, "xazheauzuheu", id)
	})

	t.Run("listing do not exist, create it", func(t *testing.T) {
		lr := new(listingRepository)
		lu := &listingUsecase{listingRepo: lr}
		...
		cID := "xazheauzuheu"
		lr.On("QueryListingByKafkaID", post.KafkaID).Return("", errors.New("")).Once()
		lr.On("PostListing", post, cID).Return(nil)
		lr.On("QueryListingByKafkaID", post.KafkaID).Return(cID, nil).Once()
